function [lagsVec, AIC, SBC, logL, HQIC] = VARlag(ENDO,maxlag,const,EXOG,lag_ex)
% ====================
...
AIC  = zeros(maxlag,1);
SBC  = zeros(maxlag,1);
HQIC = zeros(maxlag,1); % Leifi > Baett vid ur BVAR pakka
lagsVec = zeros(maxlag,1);
for i=1:maxlag
    ...
    % Leifi > Baeti við HQIC ur BVAR pakka
    HQIC(i) = -2*(logL(i)/NOBS) +  2* (NVAR*NTOTCOEFF)*log(log(NOBS))/NOBS;
...
% Ur BVAR pakka til að herma HQIC
%AIC = - 2 * llf / T + 2 * K / T;
